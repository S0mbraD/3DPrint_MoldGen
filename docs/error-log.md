...
## ERR-020: 分型横截面误选内腔回路（周长最长 ≠ 外壳分型线）

**日期**: 2026-03-31  
**严重级别**: 高  
**影响范围**: `moldgen/core/mold_builder.py` → `_get_parting_outline()`

### 错误描述
复杂模型（高面数内腔表面）与方形外壳同时被分型平面截取时，燕尾/锯齿等互锁特征沿**腔体内表面轮廓**采样，而非沿**外壳与分型面的交线**，表现为特征位置错误、穿透腔体观感或「分型未生效」。

### 根因分析
`trimesh` 对空心 `mold_solid` 做 `section()` 会得到多条 2D 折线：外圈（机匣/盒体截面）与内圈（模型空腔截面）。内圈三角剖分更密，**折线周长**常大于外框矩形；此前用「周长最长」选取回路，会系统性地选中**内腔**轮廓。

### 修复方案
1. 对 `section_2d.discrete` 每条路径计算 2D **鞋带头面积**（`_shoelace_area_2d`）与周长（`_path_perimeter_2d`）
2. **优先取面积最大的回路**作为外壳分型线；面积均退化时再 **按周长回退**
3. 分型法向 `up` 在进入 `section` 前显式单位化，减小数值误差

### 教训
- **多回路截面必须选对外壳环**：外边界在 2D 上包围区域最大，用「最大面积」比「最长周长」更符合语义
- **高分辨率内表面会欺骗简单启发式**：不能假设「更长 = 更外」
...
25. **空心体截面多回路**: 分型平面与壳体求交可产生外轮廓与腔体轮廓多条折线；互锁采样须绑定**外壳回路**（2D 面积最大者），不得用周长最长等易误判内腔的启发式
